...
def readMSWord(source_url):
    try:
        ...
        document_pages_lc = None
        document_pages_lc = UnstructuredWordDocumentLoader(
            source_url).load()  # Note: This method does not return same object as PDf loader, e.g. Doc pages not recognized. So below custom logic is built.
...
def getEmbeddingEntireDoc(documentPath, aoai_embedding_model, chunk_size=1):
    try:
        ...
        for document_page in document_pages_lc:
            ...
            if source_doc_page_content_cleansed is not None and (len(source_doc_page_content_cleansed) > 0) and (
...
def add_document_to_csv(documentPath, document_page_content_list, document_page_embedding_list, document_page_no_list,
    ...
    try:
        ...
        for i, embedding in enumerate(document_page_embedding_list):
            df = create_df(page_content=document_page_content_list[i],
