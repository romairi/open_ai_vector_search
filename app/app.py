...
aoai_text_model_deployment = aoai_embedding_models[aoai_text_model]["version"][aoai_text_model_version][
    "deployment_name"]  # Azure OpenAI deployment name
...
def getKeywordList(input_text):
    ...
    input_text = input_text.replace('?', ' ')
    input_text = input_text.replace('!', ' ')
    keyword_list = [word.lower() for word in input_text.split() if
                    word.lower() not in ['?', 'a', 'an', 'and', 'or', 'do', 'of', 'if', 'not', 'for', 'are', 'was',
                                         'were', 'is', 'can', 'have', 'has', 'there', 'their', 'the', 'how', 'why',
                                         'when', 'what', "what's", 'in', 'to', 'i', 'we', 'you']]
    return keyword_list
...
with st.container():
    def upload_button_click():
        ...
        if file_uploader is not None and len(textbox_msalias.strip()) >= ms_alias_min_length:
            ...
            # Add document pages
            response = add_document_to_csv(documentPath=local_file_path,
    ...
    with st.sidebar:
        ...
        checkbox_score = st.checkbox(label='Score', key='checkbox_score', value=False,
                                     help='Value between 0 to 100 suggesting LLM confidence for answering the question by with retrieved passage of text.')
        checkbox_similarity = st.checkbox(label='Similarity', key='checkbox_similarity', value=False,
                                          help='Similarity between the query and retrieved passage of text.')

        checkbox_page_no = st.checkbox(label='Page No', key='checkbox_page_no', value=True,
                                       help='Document page number.')
    ...
    if checkbox_show_fileupload == True:
        ...
        file_uploader = left_column_11.file_uploader(label='Upload file.', accept_multiple_files=False,
                                                     key='file_uploader_1', type=['pdf', 'docx'],
...
with st.container():
    ...
    def search_click():
        ...
        if prompt is not None and len(prompt.strip()) >= prompt_min_length and len(
                textbox_msalias.strip()) >= ms_alias_min_length:
            answer = []
            ...
            if len(answer) == 0:
                left_column.warning(
                    'No results found. Consider uploading document/s first if you are using this app for the first time for unique alias you have specified. \n Check Upload file --> Browse file --> Click Upload to get started.')
            ...
            for ans_details in answer:
                ...
                left_column.write(
                    f'<p style="font-size:16px; color:white;background-color:#7e93ff"><b>Answer</b>: {ans_details["Answer"]}</p>',
                    unsafe_allow_html=True)
                ...
                left_column.write(
                    f'<p style="font-size:14px; color:black"><b>Source</b>:<i> {os.path.basename(ans_details[f"Source"])}</i></p>',
                    unsafe_allow_html=True)

                if checkbox_similarity:
                    left_column.write(
                        f'<p style="font-size:12px; color:black"><b>Similarity</b>: {ans_details[f"Similarity"]}</p>',
                        unsafe_allow_html=True)

                if checkbox_page_no:
                    left_column.write(
                        f'<p style="font-size:12px; color:black"><b>Page No</b>: {ans_details[f"Page"]}</p>',
                        unsafe_allow_html=True)
        ...
        for i, item in enumerate(list(reversed(questions))):
            ...
            right_column.write(
                f'<p style="font-size:16px; color:black;background-color:#f0f2f6"><b>Question</b>: {question_text} </p>',
                unsafe_allow_html=True)
            ...
            for j, ans_details in enumerate(list(reversed(answers))[i]):
                ...
                if j == 0:
                    right_column.write(
                        f'<p style="font-size:14px; color:black"><b>Answer</b>: {ans_details["Answer"]}</p>',
                        unsafe_allow_html=True)
                    if checkbox_score:
                        right_column.write(
                            f'<p style="font-size:12px; color:black"><b>Score</b>: {ans_details["Score"]}</p>',
                            unsafe_allow_html=True)
                    right_column.write(
                        f'<p style="font-size:12px; color:black"><b>Content</b>: {ans_details[f"Content"]}</p>',
                        unsafe_allow_html=True)
                    right_column.write(
                        f'<p style="font-size:12px; color:black"><b>Source</b>:<i> {os.path.basename(ans_details[f"Source"])}</i></p>',
                        unsafe_allow_html=True)
                    if checkbox_similarity:
                        right_column.write(
                            f'<p style="font-size:12px; color:black"><b>Similarity</b>: {ans_details[f"Similarity"]}</p>',
